# DVT Validation Management Platform ERD 발표 자료

> 기준 문서: `DVT_데이터_테이블_정의서_v0.3.xlsx`  
> 분석 대상: `02_컬럼정의`, `ERD` 시트  
> 목적: 16개 ERD 토픽별 데이터 구조 및 테이블 기능 설명 자료
...
1. [시스템 개요](#1-시스템-개요)
2. [데이터 구조의 핵심 특징](#2-데이터-구조의-핵심-특징)
3. [16개 ERD 토픽별 설명](#3-16개-erd-토픽별-설명)
...
4. [전체 발표 연결 순서](#4-전체-발표-연결-순서)
5. [전체 소개 문구](#5-전체-소개-문구)
6. [공통 핵심 메시지](#6-공통-핵심-메시지)
7. [주요 용어 구분](#7-주요-용어-구분)
...
16개 ERD는 다음 네 개 영역으로 묶어서 설명하면 전체 업무 흐름이 명확해진다.
...
발표 중 반복해서 강조할 수 있는 핵심 메시지는 다음과 같다.
...
이 문서는 낭독용 대본보다는 **ERD 화면을 보면서 설명할 내용을 선택할 수 있는 발표 참고자료**로 사용하는 것을 권장한다.
