...
이 문서는 **설계 명세**이며 DB 마이그레이션이나 애플리케이션 변경을 실행한 결과가 아닙니다. 현재 프로토타입의 실제 DB는 D1/SQLite 테이블 3개와 JSON 상태를 사용합니다. 실제 구현 구조는 [TABLE_SPECIFICATION.md](TABLE_SPECIFICATION.md), JSON 저장 형태는 [JSON_DATA_DICTIONARY.md](JSON_DATA_DICTIONARY.md)를 확인하세요.
