...
Validation Management Platform의 73개 테이블을 14개 업무 영역으로 나누어 설명합니다. 각 영역에서 관리하는 정보와 주요 테이블의 연결을 확인할 수 있습니다.

업무 영역 간 연결을 한눈에 보려면 **[전체 데이터 구조도](./erd-overview.md)**를 참고하세요.
...
사용자 업로드 → file_scan_job (검사) → file_asset (검사 통과 후 등록)
...
사용자 업로드는 격리한 원본의 검사 회차·결과를 `file_scan_job`에 남기고, 위협 미검출 및 원본 동일성 확인 후 `file_asset`으로 등록합니다. `file_origin`은 사용자 업로드(`UPLOAD`)와 신뢰된 서버 내부 생성 파일(`SYSTEM_GENERATED`)을 구분합니다. 서버가 생성한 PDF·내보내기 파일은 업로드 검사 작업 없이 등록할 수 있으며, 생성에 사용하는 외부 업로드 파일은 먼저 검사를 통과해야 합니다.
...
| `file_scan_job` | 사용자 업로드의 격리 원본, 검사 회차·결과와 등록 파일 연결 |
| `file_asset` | 업무에 사용할 파일의 생성 출처, 저장 위치와 원본 식별 정보 |
| `evidence_link` | 업무 기록과 증적 파일의 연결 |
| `validation_project` | 검증 프로젝트의 기본 정보와 진행·종료 상태 |
| `project_activity` | 프로젝트에서 선택한 활동과 진행 상태 |
| `project_system_baseline` | 프로젝트가 검증 기준으로 채택한 시스템 승인 개정 |
...
- 사용자 업로드는 최신 검사 회차가 `COMPLETED`·`NO_THREATS_FOUND`이고, 파일 등록 및 `result_file_id` 연결까지 끝나야 사용할 수 있습니다. 검사 완료만으로 사용을 허용하지 않으며, 등록 전 파일은 첨부·다운로드·미리보기·AI 입력에서 차단합니다.
- 검사 재시도는 같은 업로드의 새 회차로 남기고 기존 판정은 보존합니다. 중복 응답으로 파일을 두 번 등록하지 않으며, 등록 완료한 업로드에는 추가 검사 회차를 만들지 않습니다. 검사한 원본과 실제 등록한 파일의 동일성을 확인합니다.
- 증적 파일은 해당 시험 수행이나 절차 기록에 연결하며, 파일이 바뀌어도 승인에 사용한 원본은 보존합니다. 절차별 증적 연결은 [IQ ERD](#erd-05)에서 확인할 수 있습니다. PDF 생성 작업과 생성된 파일의 보관 정보는 따로 관리합니다.
